...
The full B-tier option contains 813 questions: 499 multiple choice and 314 written answer. Among the 563 pre-existing B-tier source questions, 333 concise single-best-answer prompts now have four newly authored practice choices and 230 calculations, lists, multipart or identification activities, context-dependent prompts, questionable or outdated source keys, and open responses remain written answer. Practice formatting does not rewrite substantive stems or keys; the separate audited cleanup removes only extraction residue. Each converted row identifies its distractors as practice adaptations rather than historical source options.

For every new round, the game cryptographically shuffles the unseen portion of the eligible pool before applying the requested round length; it never takes the first rows from the workbook. Seen-question history is stored only on that browser as question IDs and content fingerprints, never as passwords, prompts, answers, or student responses. If fewer unseen questions remain than the requested length, the final pass is shorter; after the selection is exhausted, the next round begins a fresh pass. Students can reset history for the current set/category/mode at any time. Multiple-choice options are shuffled separately. The retry action still reshuffles only the questions marked missed or close, retaining their practice format.

The September 8 content cleanup removes 139 leaked document artifacts across 129 master-bank records before encryption, including every `GEORGIA FFA ASSOCIATION` footer that had been merged into an answer or choice. Legitimate source-organization metadata remains available with the answer feedback.
